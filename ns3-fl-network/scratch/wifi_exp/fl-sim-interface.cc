...
namespace ns3 {
    void FLSimProvider::waitForConnection() {
        ...
        NS_LOG_UNCOND("NS3: waiting for Python on port " << m_port);
        int opt = 1;
        if (setsockopt(m_server_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) < 0) {
            NS_LOG_UNCOND("setsockopt(SO_REUSEADDR) failed: " << std::strerror(errno));
        }
#ifdef SO_REUSEPORT
        if (setsockopt(m_server_fd, SOL_SOCKET, SO_REUSEPORT, &opt, sizeof(opt)) < 0) {
            NS_LOG_UNCOND("setsockopt(SO_REUSEPORT) failed: " << std::strerror(errno));
        }
#endif
        ...
        if (bind(m_server_fd, (struct sockaddr *) &m_address, sizeof(m_address)) == -1) {
            NS_LOG_UNCOND("Could not bind to port " << m_port << ": " << std::strerror(errno));
            close(m_server_fd);
            exit(-1);
        }
        if (listen(m_server_fd, 3) < 0) {
            NS_LOG_UNCOND("listen() failed: " << std::strerror(errno));
            close(m_server_fd);
            exit(-1);
        }
        ...
        if (m_new_socket < 0) {
            NS_LOG_UNCOND("accept() failed: " << std::strerror(errno));
            close(m_server_fd);
            exit(-1);
        }

        NS_LOG_UNCOND("NS3: Python connected");
    }
