...
namespace ns3 {
        ...
        Experiment::Experiment(int numClients, std::string &networkType, int maxPacketSize, double txGain, double modelSize,
                   std::string &clientDataRate, std::string &serverDataRate, bool bAsync, FLSimProvider *fl_sim_provider, FILE *fp, int round,
                   double roundMaxTimeS, double clientComputeDelayS) :
            m_networkType(networkType),
            m_maxPacketSize(maxPacketSize),
            m_txGain(txGain),
            m_modelSize(modelSize),
            m_clientDataRate(clientDataRate),
            m_serverDataRate(serverDataRate),
            m_bAsync(bAsync),
            m_flSymProvider(fl_sim_provider),
            m_fp(fp),
            m_round(round),
            m_clientComputeDelayS(clientComputeDelayS) {
    }
    ...
    Experiment::WeakNetwork(std::map<int, std::shared_ptr<ClientSession> > &clients, ns3::Time &timeOffset) {
        ...
        NodeContainer c;
        // Create numClients + 1 server
        c.Create(numClients + 1);

        // NS_LOG_UNCOND("NS3: network numClients=" << numClients << " totalNodes=" << totalNodes << " type=" << m_networkType);
        ...
        server_helper.SetAttribute("DataRate", DataRateValue(DataRate(m_serverDataRate)));
        ...
        double simStopS = 300.0;  // Default simulation stop time in seconds
        ...
        for (int j = 1; j <= numClients; j++) {
            if (clients[j - 1]->GetInRound()) {
                ...
                app->Setup(source, sinkAddress, m_maxPacketSize, m_modelSize, DataRate(m_clientDataRate));
                app->SetAttribute("ComputeDelayS", DoubleValue(m_clientComputeDelayS));
                c.Get(j)->AddApplication(app);
                app->SetStartTime(Seconds(1.));
                app->SetStopTime(Seconds(simStopS));
        ...
        Simulator::Stop(Seconds(simStopS));
        Simulator::Run();
        ...
        if (m_bAsync == false) {
            ...
            for (auto itr = sk.begin(); itr != sk.end(); itr++) {
                ...
                // NS_LOG_UNCOND(
                //         "[CLIENT_UPLINK]  " << clientAddress << " -> 10.1.1.1" << std::endl <<
