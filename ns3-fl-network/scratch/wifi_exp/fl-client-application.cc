...
namespace ns3 {
    ...
    ClientApplication::GetTypeId(void) {
        static TypeId tid = TypeId("ns3::ClientApplication")
                ...
                .AddAttribute("ComputeDelayS",
                              "Client-side compute delay after receiving full model (seconds)",
                              TypeId::ATTR_SGC,
                              DoubleValue(0.0),
                              MakeDoubleAccessor(&ClientApplication::m_computeDelayS),
    ...
    void ClientApplication::ConnectionSucceeded(Ptr <Socket> socket) {
        // NS_LOG_UNCOND("NS3: client connected id=" << socket->GetNode()->GetId() << " modelBytes=" << m_bytesModel);
    ...
    void ClientApplication::HandleRead(Ptr <Socket> socket) {
        ...
        while ((packet = socket->Recv())) {
            ...
            // Set begin time on first packet received (if not already set)
            ...
            if (pkt_count == 1 || m_bytesModelReceived % (20*1024*1024) == 0) {
            }

            if (m_bytesModelToReceive == 0)  //All bytes received, start transmitting
            {
                m_timeEndReceivingModelFromServer = Simulator::Now();
                Simulator::Schedule(Seconds(m_computeDelayS), &ClientApplication::StartWriting, this);
            }

        }
        
        if (pkt_count > 0 && m_bytesModelReceived < m_bytesModel) {
        }
    }
    ...
    ClientApplication::StartApplication(void) {
        ...
        // Set recv callback BEFORE connecting to ensure packets aren't missed
